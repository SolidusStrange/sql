...
- FORMA PARAMETRICA SIGNIFICA PEDIRLE AL USUARIO
---
CASO 1 - INFORME 1
Resumen de las diferentes fechas en que se entregaron propiedades a la Corredora de Propiedades para su arriendo o venta durante el año.
```SQL
...
FROM 
    PROPIEDAD
WHERE
    EXTRACT(YEAR FROM fecha_entrega_propiedad) = &ANIO
ORDER BY
    EXTRACT(MONTH FROM fecha_entrega_propiedad) ASC;
```
CASO 1 - INFORME 2 
Resumen de las diferentes fechas en que se arrendaron propiedades durante el año.
```SQL
...
FROM 
    PROPIEDAD_ARRENDADA
WHERE
    EXTRACT(YEAR FROM fecini_arriendo) = &ANIO
ORDER BY
    EXTRACT(MONTH FROM fecini_arriendo) ASC;
```
Si ordenáramos por mes y después por día, usamos:
```SQL
SELECT * FROM PROPIEDAD_ARRENDADA;
...
ORDER BY
    ...
    EXTRACT(DAY FROM fecini_arriendo) ASC;
```
---
CASO 2 
```SQL
...
FROM
    CLIENTE
WHERE
    ID_ESTCIVIL = 1 OR (ID_ESTCIVIL IN (3,4) AND renta_cli >= 800000) -- QUE SEAN SEPARADOS Y DIVORCIADOS Y SUELDO MAYOR QUE 800000 O SOLTEROS
ORDER BY
    APPATERNO_CLI, NOMBRE_CLI DESC; -- ORDENAR POR APELLIDO PATERNO Y NOMBRE DE CLIENTE 
```