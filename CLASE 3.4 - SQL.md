...
- Nombre que sea igual a Marco o Marta
```SQL
SELECT * FROM EMPLEADO
WHERE NOMBRE_EMP = 'MARCO' OR NOMBRE_EMP = 'MARTA';
```
- Nombre sea igual a Marca y que tengo un sueldo mayor a 500000
```SQL
SELECT * FROM EMPLEADO
WHERE NOMBRE_EMP = 'MARCO' AND SUELDO_EMP >500000;
```
---
- Entre un rango de precios
```SQL
SELECT
	numrut_emp || '-' || dvrut_emp AS RUN,
	nombre_emp || ' ' || appaterno_emp AS NOMBRE,
...
```
---
- Se puede también desde una lista
```SQL
SELECT * FROM EMPLEADO WHERE NOMBRE_EMP IN ('MARCO', 'MARTA', 'MARIA');
```
- Se puede también todo menos algo con un NOT
```SQL
SELECT * FROM EMPLEADO WHERE NOMBRE_EMP NOT IN ('MARCO', 'MARTA', 'MARIA');
```
---
- Like, se utiliza para buscar en un inicio de una palabra y el % cualquier cosa
```SQL
SELECT * FROM EMPLEADO WHERE NOMBRE_EMP LIKE 'A%'
```
- Empiece con cualquier cosa y termine en A
```SQL
SELECT * FROM EMPLEADO WHERE NOMBRE_EMP LIKE '%A'
```
- El guión bajo son espacios o dos letras, pero que termine en N
```SQL
...
```
---
**Variables de sustitución**, pide por pantalla lo que quieres teniendo en cuenta una restricción. Es case-sensitive. Upper sirve para el case-sensitive.
```SQL
SELECT * FROM EMPLEADO WHERE NOMBRE_EMP = UPPER('&NOMBRE');
```
Para un sueldo entre dos rangos que se ingresan:
```SQL
SELECT * FROM EMPLEADO WHERE SUELDO_EMP BETWEEN &RANGO1 AND &RANGO2;
```
---
REPLICAR TABLAS A PARTIR DE UNA CONSULTA SELECT
```SQL 
